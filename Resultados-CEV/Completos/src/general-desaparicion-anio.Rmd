...
transformacion <- estimacion %>% 
  mutate(replicate = stratum_name$replica,
         name_stratum = paste(stratum_name$yy_hecho, stratum_name$is_forced_dis,sep = "-")) %>%
  filter(validated) %>% 
  select(validated, N, valid_sources, n_obs, replicate, name_stratum) %>% 
  rename(stratum_name = name_stratum)

paged_table(estimacion, options = list(rows.print = 10, cols.print = 8))

```

```{r save-estimates, echo = FALSE, eval = FALSE}

arrow::write_parquet(transformacion,
                     "~/Documents/verdata-examples/Resultados-CEV/Estimacion/output-estimacion/yy_hecho.parquet")

```

```{r open-estimates, echo = TRUE}

estimacion <- arrow::read_parquet(here::here("verdata-examples/Resultados-CEV/Estimacion/output-estimacion/yy_hecho.parquet"))
...
tabla_sampler <- estimacion %>%
  ...
  separate(stratum_name, into = c("anio", "is_forced_dis"), 
           sep = "-", remove = FALSE) %>% 
  mutate(sample_number = glue("sample_{row_number()}")) %>% 
  pivot_wider(id_cols = c("replicate", "stratum_name", "anio", "n_obs"),
              values_from = N,
