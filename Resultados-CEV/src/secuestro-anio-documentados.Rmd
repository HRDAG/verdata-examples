...
la presente violación. Específicamente, se replicará la figura (inf-der) de la 
...
`is_conflict` == TRUE) o no (variable `is_conflict` == FALSE).
...
los datos documentados. Se observa entonces que el año 2003 es el año con más
secuestros documentados (`r myNum(tabla_documentada$observed[1])` víctimas), seguido 
del año 2002 (`r myNum(tabla_documentada$observed[2])` víctimas), 2001 
((`r myNum(tabla_documentada$observed[3])`), así como 2000 
(`r myNum(tabla_documentada$observed[4])`)

Ahora bien, esta documentación se refiere a víctimas de las cuales sabemos 
que efectivamente estuvieron dentro del marco del conflicto; pero, ¿qué hay de 
las víctimas sin información acerca de esta característica? Pues bien, esta
información (junto con la ya documentada) se abordará en el [próximo ejemplo sobre imputación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/output/secuestro-anio-imputado.html)
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/output-documentados/secuestro-anio-documentado.parquet"))
