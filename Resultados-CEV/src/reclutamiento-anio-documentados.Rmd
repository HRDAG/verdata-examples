---
title: "Proceso obtención víctimas documentadas"
subtitle: "Reclutamiento de niños, niñas y adolescentes desagregado por año del hecho: 1990-2017"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la figura (inf-izq) de la 
...
Adicionalmente rabajaremos las réplicas 1 a la 10. Usted como usuario puede 
trabajar con las réplicas que desee de acuerdo con los análisis que desee realizar.
De igual manera, como los ejemplos nombrados anteriormente explican, esta
información documentada se refiere a las victimas que fueron registradas 
originalmente en las bases (y en el que algunas contienen campos faltantes). 
Es decir, al contrario de los ejemplos de imputación que hemos visto y vamos a 
ver, esta documentación no cambia para ninguna réplica, por lo que si usted 
quiere obtener lo que las fuentes documentaron o registraron, puede optar por 
diferentes números de réplica y no solamente las 10 primeras que se usaron en 
...
relacionada con la categoría de edad (edad_c) que muestra si era menor o adulto.
...
Al igual que la figura del informe metodológico, la línea de color negro muestra 
los datos documentados. Se observa entonces que el año 2000 es el año con más
reclutamientos documentados (`r myNum(tabla_documentada$observed[1])` víctimas), seguido 
del año 2002 (`r myNum(tabla_documentada$observed[2])` víctimas), 1999 
((`r myNum(tabla_documentada$observed[3])`), así como 2003 
(`r myNum(tabla_documentada$observed[4])`)

Ahora bien, esta documentación se refiere a víctimas de las cuales sabemos 
que efectivamente fueron menores de edad; pero, ¿qué hay de 
las víctimas sin información acerca de esta característica? Pues bien, esta
información (junto con la ya documentada) se abordará en el [próximo ejemplo sobre imputación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/output/reclutamiento-anio-imputado.html)
en el que también graficaremos nuestros resultados a nivel documentado e imputado.

Procederemos entonces a guardar nuestros resultados (en formato *.parquet*) en
nuestra máquina local en una carpeta denominada "output-observed", ya que esta 
tabla nos servirá como insumo para el cálculo de la combinación de las imputaciones 
acorde con las reglas de *combinación* de Rubin.

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/output-documentados/reclutamiento-anio-documentado.parquet"))
