...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/reclutamiento-perpetrador-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-perpetrador-documentado.parquet"))
...
bases de datos como menores de edad (< 18 años), y que sin embargo no 
conocemos el responsable de `r myNum(tabla_combinada$observed[5])` víctimas; y como 
se indicó en el ejemplo anterior, en esta columna excluímos víctimas adultas
y víctimas que tenían campos faltantes allí. Estas últimas fueron 
...
`r myNum(tabla_combinada$observed[5])` se imputaron y por ende estas víctimas 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/reclutamiento-perpetrador-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/reclutamiento-perpetrador-imputados.parquet"))
```
