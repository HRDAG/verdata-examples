...
subtitle: "Reclutamientos a menores de edad desagregados por macroregiones de la CEV- 1990–2017"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/reclutamiento-macroregion-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-macroregion-documentado.parquet"))
...
`r myNum(tabla_combinada$imp_hi[1])` menores de edad reclutados en la macroregion de
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/reclutamiento-macroregion-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(diez_primeros_macroregions,
                     here::here("Resultados-CEV/Imputacion/output-imputados/reclutamiento-macroregion-imputados.parquet"))
```
