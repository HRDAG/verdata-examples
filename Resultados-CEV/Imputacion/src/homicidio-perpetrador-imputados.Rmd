...
subtitle: "Homicidios desagregados por perpetrador - 1985–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la figura 4 (sup-der) 
de la página 17 y la tabla 13 de la página 21 del [anexo metodológico del proyecto](https://www.comisiondelaverdad.co/anexo-proyecto-jep-cevhrdag).
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/homicidio-perpetrador-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/homicidio-perpetrador-documentado.parquet"))
...
conocemos el responsable de `r myNum(tabla_combinada$observed[8])` víctimas; y como 
se indicó en el ejemplo anterior, en esta columna excluímos víctimas no vinculadas
a este fenómeno y víctimas que tenían campos faltantes allí. Estas últimas fueron 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/homicidio-perpetrador-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/homicidio-perpetrador-imputados.parquet"))
```
