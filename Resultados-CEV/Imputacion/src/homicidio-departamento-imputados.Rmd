...
subtitle: "Homicidios desagregados por departamento del hecho - 1985–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/homicidio-departamento-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/homicidio-departamento-documentado.parquet"))
...
```
Las barras de color gris oscuro muestran los datos documentados a nivel 
departamental. Ahora bien, esta documentación se refiere a víctimas de las cuales 
sabemos que efectivamente fueron víctimas dentro del marco del conflicto armado; 
pero, ¿qué hay de las víctimas sin información acerca de esta característica? 
...
conocimiento de si estaban o no dentro de dicho marco.
...
`r myNum(diez_primeros_departamentos$imp_hi[1])` víctimas en el departamento de `r myNum(diez_primeros_departamentos$dept_name[1])`, 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/homicidio-departamento-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(diez_primeros_departamentos,
                     here::here("Resultados-CEV/Imputacion/output-imputados/homicidio-departamento-imputados.parquet"))
```
