...
y llevarán a resultados erróneos. Este último error significa que nos datos no 
se han leído a R. Si por alguna razón, usted quiere leer los datos a pesar de saber
...
Es momento de obtener una tabla con la información documentada, la misma que se 
obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/desaparicion-macrorregion-documentado.html) sobre el cálculo de víctimas documentadas para esta misma variable 
(con la función `summary_observed`). Esta información -previamente calculada- 
la importaremos desde la carpeta "output-documentados". 

```{r import-observed, echo = TRUE}

tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/desaparicion-macrorregion-documentado.parquet"))
...
aplica únicamente a esta violación. Esta indica si la víctima
...
fueron víctimas dentro del marco del conflicto armado y fueron desaparecidas de forma forzada;
pero, ¿qué hay de las víctimas sin información acerca de estas características? 
Pues bien, esta información (junto con la ya documentada) se encuentra en la 
barra azul, la cual muestra las víctimas después del proceso de imputación 
múltiple (barras azules indican el rango de imputación -intervalos de confianza-,
siendo los puntos centrales la media de la imputación), es decir, como se indicó 
anteriormente, acá estamos incluyendo aquellas víctimas para las que no 
teniamos conocimiento de si estaban o no dentro de dichos marcos.
...
`r myNum(tabla_combinada$imp_hi[1])` víctimas en la macrorregion de
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/desaparicion-macrorregion-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/desaparicion-macrorregion-imputados.parquet"))
```
