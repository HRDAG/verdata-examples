...
subtitle: "Homicidios vinculados al conflicto armado - 1985–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la tabla 2 de la página 
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/homicidio-is_conflict-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/homicidio-is_conflict-documentado.parquet"))
...
la columna **imp_mean** hay una mayor cantidad de personas, ya que estas
...
`r myNum(tabla_combinada$imp_hi[2])` víctimas vinculadas a este fenómeno,
con un promedio de `r myNum(tabla_combinada$imp_mean[2])`. Es decir, esto implica
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/homicidio-is_conflict-estimacion.html).
```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/homicidio-is_conflict-imputados.parquet"))
```
