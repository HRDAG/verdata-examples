...
subtitle: "Desapariciones forzadas desagregadas por sexo - 1985-2016"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la tabla 7 de la página 15 del
...
Importadas nuestras réplicas es momento de obtener una tabla con la información
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/desaparicion-sexo-documentado.html) sobre el cálculo de víctimas documentadas para esta misma variable 
(con la función `summary_observed`). Esta información -previamente calculada- la 
importaremos desde la carpeta "output-documentados". 

```{r import-observed, echo = TRUE}

tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/desaparicion-sexo-documentado.parquet"))
...
bases de datos como vinculadas al conflicto armado y desaparición forzada,
y que sin embargo no conocemos el sexo de `r myNum(tabla_combinada$observed[3])` 
víctimas; y como se indicó en el ejemplo anterior, en esta columna excluímos 
víctimas no vinculadas a estos fenómenos y personas que tenían campos faltantes 
allí. Estas últimas fueron objeto de la imputación estadística múltiple y por tal
razón vemos que en la columna **imp_mean** hay una mayor cantidad de víctimas ya 
que estas `r myNum(tabla_combinada$observed[3])` se imputaron y por ende estas 
personas "pasaron" a algunas de las categorías de nuestro interés. Es decir, recordemos 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/desaparicion-sexo-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/desaparicion-sexo-imputados.parquet"))
```
