...
subtitle: "Secuestros desagregados por departamento del hecho - 1990–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
figura 3 de la página 13 (inf-der) del [anexo metodológico del proyecto](https://www.comisiondelaverdad.co/anexo-proyecto-jep-cevhrdag).
...
de dicho año (`perp_change` = TRUE)^[Adicionalmente, por cuestión de análisis
se filtró por otras variables como `p_str` en el que quitamos la categoría `!= "OTRO"`.].
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/secuestro-departamento-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/secuestro-departamento-documentado.parquet"))
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/secuestro-departamento-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(diez_primeros_departamentos,
                     here::here("Resultados-CEV/Imputacion/output-imputados/secuestro-departamento-imputados.parquet"))
```
