...
subtitle: "Reclutamientos a menores de edad vinculados al conflicto armado - 1990–2017"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la tabla 3 de la página 
11 del [anexo metodológico del proyecto](https://www.comisiondelaverdad.co/anexo-proyecto-jep-cevhrdag).
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/reclutamiento-is_conflict-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-is_conflict-documentado.parquet"))
...
de víctimas documentadas de acuerdo con su relación con el conflicto armado. Y como 
se indicó en el ejemplo anterior, todas las víctimas de reclutamiento 
están relacionadas con este fenómeno, por lo que acá no vemos víctimas con
valor de cero en dicha variable. No obstante, en esta primera columna excluímos 
víctimas mayores de edad y otras víctimas que tenían campos faltantes allí.
Estas últimas fueron objeto de la imputación estadística múltiple y por tal
razón vemos que en la columna **imp_mean** hay una mayor cantidad de personas, 
ya que estas "pasaron" a algunas de las categorías de edad (que en este caso 
son menores de 18 años); Es decir, recordemos que estas nuevas columnas incluyen 
todas las víctimas, independientemente si pasaron por un proceso de imputación o no.


Así, después del proceso de imputación estadística a la edad y con un con un nivel de 
confianza del 95% se evidencia que hubo entre `r myNum(tabla_combinada$imp_lo[1])` a
`r myNum(tabla_combinada$imp_hi[1])` víctimas menores de edad vinculadas a este fenómeno,
con un promedio de `r myNum(tabla_combinada$imp_mean[1])`. Es decir, esto implica
que este promedio es la mejor estimación puntual respecto al número de víctimas de 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/reclutamiento-is_conflict-estimacion.html).

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/reclutamiento-is_conflict-imputados.parquet"))
```

## Ejercicio {-}

Acorde con la anterior interpretación, ¿cómo interpretaría los resultados 
si incluímos otras variables?