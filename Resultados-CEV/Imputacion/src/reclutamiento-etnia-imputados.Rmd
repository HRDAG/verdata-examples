...
subtitle: "Reclutamiento de Niños, Niñas y Adolescentes desagregados por etnia - 1990–2017"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
(panorama reclutamiento), procederemos a crear, transformar y/o filtrar algunas variables. 
...
de dicho año (`perp_change` = TRUE). Además se generan otras variables para la 
categoría de edad y etnia.
...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/reclutamiento-etnia-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-etnia-documentado.parquet"))
...
print(replicas_filtradas$etnia2)
...
tabla_combinada <- tabla_combinada %>% 
  select("etnia2", observed, obs_prop_na, obs_prop, imp_lo, imp_mean, imp_hi,
...
bases de datos como menores de edad, y que sin embargo no 
conocemos la etnia de `r myNum(tabla_combinada$observed[3])` víctimas; y como 
se indicó en el ejemplo anterior, en esta columna excluímos personas mayores 
de edad y personas que tenían campos faltantes allí. Estas últimas fueron 
...
"pasaron" a algunas de las categorías de nuestro interés (edades). Es decir, recordemos 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/reclutamiento-etnia-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/reclutamiento-etnia-imputados.parquet"))
```
