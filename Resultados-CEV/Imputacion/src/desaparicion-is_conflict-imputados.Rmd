...
documentada, la misma que se obtuvo en el [ejemplo sobre víctimas documentadas](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Documentados/output/desaparicion-is_conflict-documentado.html) sobre el cálculo de víctimas documentadas para 
...
tabla_documentada <- arrow::read_parquet(here::here("Resultados-CEV/Documentados/output-documentados/desaparicion-is_conflict-documentado.parquet"))

tabla_documentada <- tabla_documentada %>% 
  rename(is_conflict_dis_rep = is_conflict_dis)

paged_table(tabla_documentada, options = list(rows.print = 10, cols.print = 5))

```

Posterior a esto^[Hemos cambiado el nombre de la variable para que no existan
conflictos respecto a la próxima función] se aplica la función de `combine_replicates` que,
...
tabla_combinada <- verdata::combine_replicates("desaparicion", 
                                               tabla_documentada,
                                               replicas_filtradas, 
                                               strata_vars = 'is_conflict_dis_rep', 
...
al conflicto y desaparición forzada y víctimas que tenían campos faltantes allí.
...
ya que estas `r myNum(tabla_combinada$observed[3])` se imputaron y por ende estas
víctimas "pasaron" a algunas de las categorías de nuestro interés; Es decir, recordemos 
...
confianza del 95% se evidencia que hubo entre `r myNum(tabla_combinada$imp_lo[2])` a
`r myNum(tabla_combinada$imp_hi[2])` víctimas desaparecidas de forma forzada
vinculadas al conflicto armado, con un promedio de `r myNum(tabla_combinada$imp_mean[2])`. 
Es decir, esto implica que este promedio es la mejor estimación puntual 
respecto al número de víctimas de esta categoría, no obstante, hay que tener en 
...
ejemplo) en contraste con los patrones de violencia a partir de la [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/desaparicion-is_conflict-estimacion.html)

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_combinada,
                     here::here("Resultados-CEV/Imputacion/output-imputados/desaparicion-is_conflict-imputados.parquet"))
```

## Ejercicio {-}

Acorde con la anterior interpretación, ¿cómo interpretaría los resultados 
si incluímos una variable como año?