...
para el anexo metodológico, procederemos a transformar y/o filtrar algunas variables. 
...
Al igual que la figura del anexo metodológico, la línea de color negro muestra 
los datos documentados. Se observa entonces que el año 2002 es el año con más
homicidios documentados (`r myNum(tabla_documentada$observed[1])` víctimas), 
seguido del año 2001 (`r myNum(tabla_documentada$observed[2])` víctimas), 2000 
((`r myNum(tabla_documentada$observed[3])`), así como 2003 
(`r myNum(tabla_documentada$observed[4])`).

Ahora bien, esta documentación se refiere a víctimas sobre las que sabemos 
que estuvieron dentro del marco del conflicto; pero, ¿qué pasa con las víctimas 
sin información acerca de esta característica? Pues bien, esta
información (junto con la ya documentada) se abordará en el [próximo ejemplo sobre imputación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/homicidio-anio-imputado.html) en el que también graficaremos nuestros resultados a nivel 
documentado e imputado.

Por último, guardamos los resultados (en formato *.parquet*) en una carpeta 
denominada "output-observed", ya que esta tabla nos servirá como insumo para el 
cálculo de la combinación de las imputaciones (o réplicas) (réplicas) acorde con las reglas de 
*combinación* de Rubin.

```{r save-observed, echo = TRUE}

arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/homicidio-anio-documentado.parquet"))
