...
subtitle: "Homicidios desagregados por sexo - 1985–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación (desagregado por sexo). Específicamente, se replicará la 
tabla 8 de la página 15 del [anexo metodológico del proyecto](https://www.comisiondelaverdad.co/anexo-proyecto-jep-cevhrdag).
...
será la variable de **sexo**; 4) le sigue el argumento de **conflict_filter** 
...
Además, evidenciamos que no conocemos el sexo de `r myNum(tabla_documentada$observed[3])`
víctimas de esta homicidio, por lo que en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/homicidio-sexo-imputado.html) veremos que estas fueron objeto de la imputación estadística múltiple y 
...
"pertenece al conflicto"). Seguido de esto vemos que `r myNum(tabla_documentada$observed[1])`
personas fueron documentadas como víctimas hombres mientras que `r myNum(tabla_documentada$observed[2])`
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/homicidio-sexo-documentado.parquet"))
