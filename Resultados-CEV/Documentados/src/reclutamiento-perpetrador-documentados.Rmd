...
subtitle: "Reclutamiento de Niños, Niñas y Adolescentes desagregados por perpetrador - 1990–2017"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la figura 4 (inf-izq) 
de la página 17 y la tabla 14 de la página 22 del [anexo metodológico del proyecto](https://www.comisiondelaverdad.co/anexo-proyecto-jep-cevhrdag).
...
de diferentes fuentes, es menester que sepa si han sido modificados o no, para 
...
y llevarán a resultados erróneos. Este último error significa que los datos no 
se han leído a R. Si por alguna razón usted quiere leer dicha información a pesar
de saber que no son los mismos datos originalmente publicados, puede cambiar el
...
será la variable de **p_str**; 4) le sigue el argumento de **conflict_filter** 
...
Cabe aclarar que todas las víctimas de esta violación están vinculadas al conflicto 
armado.
...
grafica_tabla <- tabla_documentada %>% 
  filter(!p_str %in% c('OTRO', 'EST', 'multiple')) %>% 
  na.omit()

grafica <- ggplot(grafica_tabla,
...
documentadas como menores de edad, es decir, excluimos aquellas víctimas adultas
y también a aquellas en las que existía ambigüedad respecto a esta característica 
(es decir, que presentan campos faltantes). Además, evidenciamos que no conocemos
el perpetrador de `r myNum(tabla_documentada$observed[8])` víctimas de esta 
violación, por lo que en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/reclutamiento-perpetrador-imputado.html) veremos que estas fueron objeto de la imputación estadística múltiple y 
que por ende estas víctimas "pasaron" a ser parte de una de estas categorías
(respecto nuestra variable de interés y a alguna de las categorías de edad"). 
Seguido de esto vemos que `r myNum(tabla_documentada$observed[3])`
personas fueron documentadas como víctimas de las FARC-EP mientras que 
`r myNum(tabla_documentada$observed[6])` fueron reclutadas por los paramilitares.

Adicionalmente omitimos ciertos actores, ya que, acorde con los investigadores de
la CEV, no es muy común o típico que estos grupos recluten de forma forzada 
a los menores de edad, sumado a que por lo general estas víctimas no las toman 
con el objetivo de combatir. Puede explorar esta explicación más a fondo en el 
anexo Final de dicha institución (**Capítulo 1: No es un mal menor. Niños, 
niñas y adolescentes en el conflicto armado**).
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-perpetrador-documentado.parquet"))
