...
Después de importar la información es momento  de obtener la información de las 
víctimas documentadas por nuestra variable de interés. 
...
aplica únicamente a esta violación. Esta indica si la víctima fue desaparecida de forma "forzada" (`forced_dis` == TRUE) o no (`forced_dis` == FALSE). 
...
```
Entonces, lo primero que podemos evidenciar que estas son las víctimas desaparecidas
de forma forzada que fueron documentadas dentro del marco del conflicto armado, 
es decir, excluimos aquellas víctimas no relacionadas y también a aquellas en la
que existía ambigüedad de si hacían parte o no de dicho fenómeno (es decir, 
que presentan campos faltantes). Además, evidenciamos que no conocemos el perpetrador de `r myNum(tabla_documentada$observed[8])` víctimas de esta violación, por lo que en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/desaparicion-perpetrador-imputado.html) veremos que estas fueron objeto de la imputación estadística múltiple y 
...
"pertenece al conflicto"). Seguido de esto vemos que `r myNum(tabla_documentada$observed[6])`
personas fueron documentadas como víctimas de los grupos paramilitares mientras que `r myNum(tabla_documentada$observed[3])`
fueron víctimas de las FARC-EP.
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/desaparicion-perpetrador-documentado.parquet"))
