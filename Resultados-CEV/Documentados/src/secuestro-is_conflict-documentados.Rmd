...
subtitle: "Secuestros vinculados al conflicto armado - 1990–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
la presente violación. Específicamente, se replicará la tabla 4 de la página 
...
Siguiendo lo anterior, la función `filter_standard_cev` nos permite transformar
o filtrar nuestra información. Por ejemplo, las víctimas que se documentaron 
...
existir después de dicho año, por lo que el tercer argumento será igual a TRUE
(`perp_change` = TRUE).
...
será la variable de **is_conflict**; 4) le sigue el argumento de **conflict_filter** 
...
Para nuestro ejemplo optaremos por dimensionar todas las víctimas **documentadas**
vinculadas al conflicto acorde con las fuentes de información.
...
de víctimas documentadas de acuerdo con su relación con el conflicto armado. 
Segundo, vemos `r myNum(tabla_documentada$observed[3])` víctimas de las 
que no sabemos su relación respecto a este fenómeno, es decir, no sabemos si en
realidad fueron víctimas dentro del marco del conflicto, por lo 
que en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/secuestro-is_conflict-imputado.html) veremos que estas fueron objeto de la imputación
estadística múltiple y que por ende estas víctimas "pasaron" a ser parte de una 
de estas dos categorías. Seguido de esto vemos que `r myNum(tabla_documentada$observed[1])` 
personas no fueron víctimas dentro del marco del conflicto y que por ende,
si usted como usuario quiere realizar un análisis relacionado netamente a este 
fenómeno, no es correcto incluir estas víctimas, ya que estos secuestros
ocurrieron en otros contextos (de acuerdo con las fuentes que las documentaron). 
Pero, por otro lado, vemos que `r myNum(tabla_documentada$observed[2])` 
personas sí fueron víctimas dentro de este marco (según las fuentes de información).
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/secuestro-is_conflict-documentado.parquet"))
