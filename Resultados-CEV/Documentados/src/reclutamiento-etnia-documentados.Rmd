...
centrarnos en un análisis más específico, tal como el realizado
para el anexo metodológico (panorama reclutamiento), procederemos a transformar y/o filtrar 
algunas variables. 
...
documentadas como menores de 18 años, es decir, excluimos aquellas 
víctimas mayores de edad y también a aquellas en las que existía ambigüedad de 
sobre esta característica (es decir, que presentan campos faltantes), por lo que 
en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/reclutamiento-etnia-imputado.html) veremos que estas fueron objeto de la imputación
estadística múltiple y que por ende estas víctimas "pasaron" a ser parte de
alguna de las categorías de edad).

Así, evidenciamos que no conocemos la etnia de `r myNum(tabla_documentada$observed[3])`
víctimas de esta violación. Seguido de esto vemos que `r myNum(tabla_documentada$observed[2])`
personas fueron documentadas como víctimas mestizas (52%) mientras que `r myNum(tabla_documentada$observed[1])`
fueron víctimas étnicas (NARP+indígena+ROM).
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-etnia-documentado.parquet"))
