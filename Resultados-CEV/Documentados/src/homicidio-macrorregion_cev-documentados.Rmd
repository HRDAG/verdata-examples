...
subtitle: "Homicidios desagregados por macrorregion de la CEV - 1985–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
por lo que en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/homicidio-macrorregion-imputado.html) veremos que estas fueron objeto de la imputación
...
macroregion no presenta campos faltantes). Seguido de esto vemos que (por ejemplo) 
`r myNum(tabla_documentada$observed[2])` personas fueron víctimas 
en la macrorregion de `r myNum(tabla_documentada$macroregion[2])` mientras que 
`r myNum(tabla_documentada$observed[4])` fueron víctimas en la macroregion de 
`r myNum(tabla_documentada$macroregion[4])`
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/homicidio-macrorregion-documentado.parquet"))
