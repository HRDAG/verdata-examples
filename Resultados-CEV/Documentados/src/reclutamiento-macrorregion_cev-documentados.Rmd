...
replicas_datos <- verdata::read_replicates(here::here("verdata-csv/reclutamiento"),
                                           "reclutamiento", c(1:10))
...
En tal sentido la función `filter_standard_cev` nos permite transformar o filtrar
nuestra información. Por ejemplo, las víctimas que se documentaron como víctimas 
...
adicionales como quinquenio, más categorías de edades, etnia, etc.
...
en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/reclutamiento-macrorregion-imputado.html) veremos que estas fueron objeto de la imputación
estadística múltiple y que por ende estas víctimas "pasaron" a ser parte de
alguna de las categorías de edad (menor o adulto, ya que nuestra variable de 
macrorregion no presenta campos faltantes). Seguido de esto vemos que 
`r myNum(tabla_documentada$observed[1])` menores fueron víctimas 
en la macrorregion de `r myNum(tabla_documentada$macroregion[1])` 
mientras que `r myNum(tabla_documentada$observed[2])`
fueron reclutadas en la macrorregion de `r myNum(tabla_documentada$macroregion[2])`
...
arrow::write_parquet(tabla_documentada,
                     here::here("Resultados-CEV/Documentados/output-documentados/reclutamiento-macrorregion-documentado.parquet"))
