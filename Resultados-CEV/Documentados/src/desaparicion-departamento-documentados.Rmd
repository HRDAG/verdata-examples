...
subtitle: "Desapariciones forzadas desagregadas por departamento del hecho - 1985–2016"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
figura 3 de la página 13 (sup-izq) del [anexo metodológico del proyecto](https://www.comisiondelaverdad.co/anexo-proyecto-jep-cevhrdag).
...
aplica únicamente a la presente violación. Esta indica si la víctima fue 
...
las que existía ambigüedad de si hacían parte o no de dichos fenómenos 
(es decir, que presentan campos faltantes), por lo que en el siguiente [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Imputacion/output/desaparicion-departamento-imputado.html) veremos que estas fueron objeto de la imputación
estadística múltiple y que por ende estas víctimas "pasaron" a ser parte de una
de estas categorías (ya que nuestra variable de 
departamento no presenta campos faltantes). Seguido de esto vemos que 
`r myNum(diez_primeros_departamentos$observed[1])` personas fueron víctimas 
de desaparición forzada en el departamento de `r myNum(diez_primeros_departamentos$dept_name[1])` 
mientras que `r myNum(diez_primeros_departamentos$observed[2])`
fueron víctimas en el departamento de `r myNum(diez_primeros_departamentos$dept_name[2])`
...
arrow::write_parquet(tabla_departamento,
                     here::here("Resultados-CEV/Documentados/output-documentados/desaparicion-departamento-documentado.parquet"))
