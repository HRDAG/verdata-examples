...
por año del hecho (1990–2018).

## Importando resultados estratificación {-}

En este [ejemplo](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/output/secuestro-anio-estratificacion.html)
...
results <- readRDS(here::here("Resultados-CEV/output-estratificacion/secuestro-yy_hecho-sexo.rds"))
...
estimación para víctimas secuestradas -desagregadas por año del hecho- es 
combinar nuestras estimaciones. Esto lo haremos a través de nuestra función
`combine_estimates` el cual permite obtener los intervalos creibles (no de 
confianza). Pero antes, debemos realizar una pequeña transformación a nuestra 
anterior tabla `union_mse`, esto con el fin de obtener nuestros resultados 
desagregados esta característica (y proceder luego a combinar). Esto lo
presentaremos a continuación:

```{r combinacion-transformacion, echo = TRUE}

tabla_sampler <- union_mse %>%
    separate(stratum_name, into = c("yy_hecho", "sexo"), 
             sep = "-", remove = FALSE) %>% 
...
separa la columna de `stratum_name` en las variables originales de sexo y 
año del hecho, luego agrupamos por nuestra columna `stratum_name` y cada una de
nuestras 10 réplicas. 
...
habíamos explicado anteriormente, cada estrato (por ejemplo *1990-HOMBRE* para 
la réplica (R1) contiene 1000 muestras aleatorias de la distribución posterior,
por lo que cada valor de este *N* se expanderá a cada columna de `sample_` 
y por tanto, esas serán 1000 columnas, desde `sample_1` hasta `sample_1000`.
...
En otras palabras, primero agrupamos por departamento del hecho y réplica, es 
decir -por ejemplo- agrupar por el año de 1990 y la réplica 1, 1990 y réplica 10,
y así sucesivamente. Cuando tengamos este tipo de agrupación (como lo vemos en 
la *tabla_agrupacion*) tomaremos -por ejemplo- la réplica 1 ("R1") y la categoría
o año de 1990 y sumaremos los valores para el `sample_` (es como tomar: 
`ejemplo <- tabla_sampler %>% filter(replicate == "R1", yy_hecho == "1990")` y 
sumar: `sum(ejemplo$sample_1)`, dándonos lo que nos está arrojando la primera 
fila de la columna `sample_1`, es decir, `r myNum(tabla_agrupacion$sample_1[1])`).

Sumado a esto, volveremos a transformar nuestros datos de ancho a largo, 
es decir, volveremos a una estructura muy parecida a *tabla_sampler*, pero 
esta vez nuestro *N* muestra la suma de las muestras de la distribución posterior
...
Para finalizar, vemos que esta información está desagregada por año del hecho y 
réplica, pero, como sabemos, deseamos ver los resultados únicamente por año del
hecho. Por tal razón agruparemos esta información por dicha variable (que ahora
se denomina `stratum_name`): 
...
`combine_estimates` a cada una de nuestras categorías. Por último -con 
`bind_cols`- unimos estos resultados con las variables de agrupación, es decir,
con `yy_hecho` que la denominamos `stratum_name`. 

Seguido de esto importaremos nuestros resultados a nivel documentado e imputado:

```{r datos-doc-imp, echo = TRUE}

tabla_doc_imp <- arrow::read_parquet(here::here("Resultados-CEV/output-imputados/secuestro-anio-imputados.parquet"))
