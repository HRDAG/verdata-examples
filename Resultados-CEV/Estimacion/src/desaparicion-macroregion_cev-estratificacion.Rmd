...
Para esto necesitaremos un insumo adicional que serán las macroregiones que se 
generaron en la Comisión de la Verdad (CEV). Procederemos a importar dicho insumo 
que se se encuentra en la carpeta "Recursos" a partir del paquete `readr`:
...
como víctimas de la ex-guerrilla FARC-EP en años posteriores a 2016 pasaron a 
...
como queremos estimar el subregistro de la presente violación por macroregion,
se estratificará por quinquenio y dicha variable. Sin embargo, es necesario filtrar
primero por las variables de "pertenece al conflicto" `is_conflict` y la 
anteriormente mencionada `is_forced_dis`.
...
las víctimas de la macroregion Centro-Andina que fueron desaparecidas de forma 
...
tiempo y la macroregion. Nuevamente, este solo es un ejemplo de nuestra forma 
...
saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/quinquenio-macroregion-desaparicion.rds"))

```

El siguiente paso es realizar el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/desaparicion-macroregion-estimacion.html).
