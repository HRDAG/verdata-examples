...
subtitle: "Reclutamientos a menores de edad desagregados por departamento del hecho - 1990–2017"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
proceso de estimación del total de víctimas por departamento del hecho (1990–2017).
...
nuevas variables, tal como la variable de "quinquenio" la cual será clave 
para el proceso de estratificación:
...
como queremos estimar el subregistro de la presente violación por departamento del hecho,
se estratificará por quinquenio y departamento del hecho. Es importante que usted 
...
que en nuestro caso es **replicas_filtradas**, es decir, la información previamente 
filtrada.
...
**quinquenio** y **dept_code_hecho** para cada réplica. El objeto **schema** contiene una 
cadena de caracteres con los nombres de las variables en el *data frame*. Luego, 
como se mencionó, usamos la tabla **replicas_filtradas** como primer argumento y el 
...
elemento 100 de la lista contiene las víctimas del departamento de Boyacá que 
fueron reclutadas de forma forzada durante el quinquenio 2005-2009, presentes 
en la réplica 1 (ver el objeto **victimas_estrato** y **victimas_estrato_nombre**):
...
saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/quinquenio-departamento-reclutamiento.rds"))

```

El siguiente paso es realizar el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/reclutamiento-departamento-estimacion.html).