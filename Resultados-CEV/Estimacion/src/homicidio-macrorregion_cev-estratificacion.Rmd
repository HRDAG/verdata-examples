---
title: "Proceso estratificación para estimación de víctimas desagregadas por macrorregiones de la CEV"
...
proceso de estimación del total de víctimas por macrorregion de la CEV (1985–2018). 
Para esto necesitaremos un insumo adicional que serán las [macrorregiones](https://web.comisiondelaverdad.co/actualidad/noticias/comision-de-la-verdad-inicia-el-despliegue-exploratorio-en-macroregiones#:~:text=Las%20nueve%20macrorregiones%20definidas%20son,Magdalena%2C%20Sucre%20y%20San%20Andr%C3%A9s.) 
...
Después de esto pasaremos a autenticar e importar la base de datos, esto a 
...
confirmar <- verdata::confirm_files(replicates_dir = here::here("verdata-parquet/homicidio"),
                                    violation = "homicidio", 
...
replicas_datos <- verdata::read_replicates(replicates_dir = here::here("verdata-parquet/homicidio"),
                                           violation = "homicidio", 
                                           replicate_nums = c(1:10),
                                           version = "v1")
...
información. Por ejemplo, esta función permite obtener nuevas variables de tiempo 
(como quinquenio), variables de edad (como "edad_c"), entre otros. 
Adicionalmente filtramos por los homicidios ocurridos dentro del marco del 
conflicto armado (is_conflict == 1) y unimos esta información con las 
macrorregiones:
...
por macrorregiones de la CEV y quinquenio). Es importante 
que usted como usuario vea que este proceso es netamente artesanal, es decir,
usted puede usar su propio código o funciones para realizar este proceso que, 
en nuestro caso, será a través de una función previamente creada (fuera del 
paquete `verdata`) para facilitar este ejercicio:
...
schema <- ("replica,quinquenio,macroregion")
...
**quinquenio** y **macroregion** para cada réplica. El objeto **schema** contiene 
una cadena de caracteres con los nombres de las variables en el *data frame*. 
Luego, como se mencionó, usamos la tabla **replicas_estratos** como primer 
argumento y el objeto **schema** como segundo. Lo que obtenemos es lo siguiente:

El objeto **listas** contiene dos listas. La primera, llamada **strata_data** que
contiene las víctimas que fueron víctimas de este hecho según sus características
para cada una de las réplicas. Por ejemplo, el elemento 25 de la lista contiene
las víctimas de la macrorregion Centro-Andina que fueron asesinadas
en el quinquenio de 1995-1999 presentes en la réplica 1 (ver el objeto 
**victimas_estrato**):
...
estrato. Siguiendo el mismo ejemplo, el elemento 25 de **stratification_vars** 
contiene una columna con la respectiva réplica, una columna con la variable de
tiempo y la macrorregion. Nuevamente, este solo es un ejemplo de nuestra forma 
de estratificar, usted puede hacerlo de otra manera. La idea principal es agrupar
las víctimas por las variables del estrato y las réplicas que esté usando.

```{r stratify-continuacion3, echo = TRUE}

nombres <- listas[["stratification_vars"]]

victimas_estrato_nombre <- nombres[[25]]
...
próximo ejemplo sobre el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/homicidio-macrorregion-estimacion.html).

```{r save-estratify, echo = TRUE}

saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/quinquenio-macrorregion-homicidio.rds"))
