...
subtitle: "Homicidios desagregados por edad - 1985–2018"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
...
desagregadas por edad (1985–2018). 
...
como queremos estimar el subregistro de la presente violación por edad,
se estratificará por año del hecho y una nueva categoría de edad. Para empezar,
es necesario filtrar por las variables de "pertenece al conflicto", `is_conflict` ya que 
la idea principal es estimar el subregistro de víctimas vinculadas al conflicto 
armado.
...
schema <- ("replica,yy_hecho,edad_categoria")
...
**yy_hecho** y **edad_c** para cada réplica. El objeto **schema** contiene una 
...
contiene las víctimas que fueron víctimas de este hecho respecto a estas características 
de estratificación. Por ejemplo, el elemento 5 de la lista contiene las víctimas
adultas que fueron asesinadas en 1987 presentes en la réplica 1 (ver objetos 
**victimas_estrato** y **victimas_estrato_nombre**):

```{r stratify-continuacion2, echo = TRUE}

datos <- listas[["strata_data"]]

victimas_estrato <- datos[[5]]
...
victimas_estrato_nombre <- nombres[[5]]
...
saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/yy_hecho-edad-homicidio.rds"))

```

El siguiente paso es realizar el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/homicidio-edad-estimacion.html).
