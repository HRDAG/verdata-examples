...
proceso de estimación del total de víctimas por perpetrador (1985–2016). 
...
se han leído a R. Si por alguna razón, usted quiere leer dicha información a 
pesar de saber que no son los mismos datos originalmente publicados, puede cambiar
el argumento `crash` a FALSE, y, en ese caso, podrá ver los datos junto con el
...
como queremos estimar el subregistro de la presente violación por el presunto responsable,
se estratificará por año del hecho, la variable de "es desaparición forzada" y 
la variable de perpetrador. Sin embargo, es necesario filtrar primero por las 
...
schema <- ("replica,yy_hecho,is_forced_dis,p_str")
...
**yy_hecho**, **is_forced_dis** y **p_str** para cada réplica. El objeto 
...
para cada una de las réplicas. Por ejemplo, el elemento 160 de la lista contiene
las personas víctimas de otros grupos que fueron desaparecidas de forma forzada 
en el año 2007 presentes en la réplica 1 (ver el objeto **victimas_estrato**):

```{r stratify-continuacion2, echo = TRUE}

datos <- listas[["strata_data"]]

victimas_estrato <- datos[[160]]
...
estrato. Siguiendo el mismo ejemplo, el elemento 160 de **stratification_vars** 
contiene una columna con la respectiva réplica, una columna con la variable de
tiempo, la variable de es "desaparición forzada" y la variable de perpetrador. 
...
victimas_estrato_nombre <- nombres[[160]]
...
próximo ejemplo sobre el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/desaparicion-perpetrador-estimacion.html).

```{r save-estratify, echo = TRUE}

saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/yy_hecho-is_forced_dis-perpetrador-desaparicion.rds"))
