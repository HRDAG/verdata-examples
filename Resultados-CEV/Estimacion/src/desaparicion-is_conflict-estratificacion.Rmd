...
proceso de estimación del total de víctimas vinculadas al conflicto armado (1985–2016).
...
como queremos estimar el subregistro de la presente violación para el total de
víctimas vinculadas al conflicto armado, se estratificará por año del hecho y 
la variable de "es desaparición forzada" (tal como en el ejemplo completo para 
estimaciones por año). Sin embargo, es necesario filtrar primero por las 
variables de "pertenece al conflicto" `is_conflict` y la anteriormente mencionada
`is_forced_dis`.
...
**yy_hecho** e **is_forced_dis** para cada réplica. El objeto 
...
para cada una de las réplicas. Por ejemplo, el elemento 1 de la lista contiene
las personas que fueron desaparecidas de forma forzada (vinculadas al conflicto
armado) en el año de 1985 presentes en la réplica 1 (ver el objeto **victimas_estrato**):

```{r stratify-continuacion2, echo = TRUE}

datos <- listas[["strata_data"]]

victimas_estrato <- datos[[1]]
...
estrato. Siguiendo el mismo ejemplo, el elemento 1 de **stratification_vars** 
contiene una columna con la respectiva réplica, una columna con la variable de
tiempo y la variable de es "desaparición forzada".
...
victimas_estrato_nombre <- nombres[[1]]
...
próximo ejemplo sobre el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/desaparicion-is_conflict-estimacion.html).

```{r save-estratify, echo = TRUE}

saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/yy_hecho-is_forced_dis-is_conflict-desaparicion.rds"))
