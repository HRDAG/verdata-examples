---
title: "Proceso estratificación para estimaciones por año del hecho y sexo"
...
proceso de estimación del total de víctimas por año del hecho y también por sexo
(1990–2018) ya que, en comparación con otros ejercicios, el presente ejemplo nos 
permitirá estimar el subregistro de víctimas desagregadas por estas variables sin
necesidad de realizar dos tipos de estratificaciones.
...
Adicionalmente, como queremos estimar únicamente a las víctimas vinculadas al 
conflicto armado, filtraremos por aquellas personas víctimas dentro de este marco 
(is_conflict == 1):
...
como queremos estimar el subregistro de la presente violación por sexo y también
por año del hecho, se estratificará por dichas variables. Es importante que usted como usuario 
...
que en nuestro caso es **replicas_datos**, es decir, la información previamente 
filtrada.
...
schema <- ("replica,yy_hecho,sexo")

listas <- stratify(replicas_datos, schema)
...
**yy_hecho** y **sexo** para cada réplica. El objeto **schema** contiene una 
cadena de caracteres con los nombres de las variables en el *data frame*. Luego, 
como se mencionó, usamos la tabla **replicas_datos** como primer argumento y el 
...
-desagregado por sexo- para cada una de las réplicas. Por ejemplo, el 
elemento 151 de la lista contiene las víctimas hombres en el año 2007 presentes 
en la réplica 2 (ver el objeto **victimas_estrato**):
...
estrato. Siguiendo el mismo ejemplo, el elemento 151 de **stratification_vars** 
contiene una columna con la réplica anteriormente dicha, una columna con el año
(2007) y el sexo. Nuevamente, este solo es un ejemplo de nuestra forma 
...
próximo ejemplo sobre el proceso de [estimación y combinación](https://github.com/HRDAG/CO-examples/blob/main/Resultados-CEV/Estimacion/output/secuestro-anio_sexo-estimacion.html).

```{r save-estratify, echo = TRUE}

saveRDS(listas, 
        here::here("Resultados-CEV/Estimacion/output-estimacion/yy_hecho-sexo-secuestro.rds"))
